...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function moveGallery() {
        ...
        if (currentImageIndex === images.length - 1) {
            // Se sim, espera a transição terminar para resetar
            setTimeout(() => {
                // Remove a transição para o reset ser invisível
                galleryContainer.style.transition = 'none';
    
                // Reseta o índice e a posição
                currentImageIndex = 0;
                galleryContainer.style.transform = `translateX(0%)`;
    
                // Adiciona a transição de volta para as próximas animações
                setTimeout(() => {
                    galleryContainer.style.transition = 'transform 0.5s ease-in-out';
                }, 50); // Um pequeno delay é necessário para o navegador renderizar a mudança
            }, 500); // O tempo (500ms) deve corresponder à duração da transição no CSS
        }
    
    }

    // Define o intervalo para a transição das imagens (a cada 5 segundos)
    setInterval(moveGallery, 4000);
